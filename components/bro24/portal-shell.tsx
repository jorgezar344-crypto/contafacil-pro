...
export function PortalShell({ children, title, description }: { children: React.ReactNode; title: string; description?: string }) {
  ...
  const workspace = useWorkspace(); const query = typeof window === "undefined" ? "" : window.location.search; const href=(value:string)=>`${value}${query}`;
  return <div className="min-h-screen bg-[#f4f6f8]">
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r border-black/20 bg-[#15181d] px-4 py-6 text-white lg:flex">
      <BrandLogo /><div className="mt-8 space-y-1">{items.map(([label, to, Icon]) => <Link key={to} href={href(to)} className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold transition ${pathname === to ? "bg-[#ff6a00] text-white" : "text-slate-300 hover:bg-white/10 hover:text-white"}`}><Icon size={18}/>{label}</Link>)}</div>
    ...
    <div className="lg:pl-64"><header className="sticky top-0 z-20 flex min-h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur sm:px-7"><div className="lg:hidden"><BrandLogo compact /></div><div className="hidden lg:block text-xs text-slate-500">BRO24 Contable <span className="mx-2 text-slate-300">/</span>{title}</div><div className="flex items-center gap-2">{workspace?.loading?<span className="text-xs">Cargando contexto…</span>:workspace?.workspace?<><select aria-label="Empresa activa" value={workspace.workspace.company.id} onChange={e=>workspace.change(e.target.value,null)} className="max-w-40 rounded border p-1 text-xs">{workspace.workspace.companies.map((c:any)=><option key={c.id} value={c.id}>{c.legal_name}</option>)}</select><select aria-label="Periodo activo" value={workspace.workspace.period?.id||""} onChange={e=>workspace.change(undefined,e.target.value||null)} className="max-w-32 rounded border p-1 text-xs"><option value="">Sin periodo</option>{workspace.workspace.periods.map((p:any)=><option key={p.id} value={p.id}>{p.month}/{p.year}</option>)}</select></>:<span className="text-xs text-slate-500">{workspace?.error==="UNAUTHORIZED"?"Sesión vencida":"Sin empresa"}</span>}<button aria-label="Notificaciones" className="rounded-lg p-2 text-slate-600 hover:bg-slate-100"><Bell size={19}/></button></div></header>

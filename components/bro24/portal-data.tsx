...
export function usePortalDocuments() {
  ...
  const workspace=useWorkspace(); const company=workspace?.workspace?.company?.id,period=workspace?.workspace?.period?.id,errorCode=workspace?.error,href=workspace?.href; const refresh = useCallback(async () => { setLoading(true); try { if(!company||!period||!href)throw new Error(errorCode||"CONTEXT_REQUIRED");const response=await fetch(href("/api/app-data",{page:"1",page_size:"20"}),{cache:"no-store"}); const data=await response.json(); if(!response.ok) throw new Error(data.code||data.error); setDocuments(Array.isArray(data.documents)?data.documents:[]); setError(""); } catch { setDocuments([]); setError(errorCode||"No fue posible cargar los documentos disponibles."); } finally { setLoading(false); } },[company,errorCode,href,period]);
